...
const Preloader = () => {
    ...
    useEffect(() => {
        const animate = async () => {
            ...
            // Animate text fade-in with bounce
            await textControls.start({
                opacity: 1,
                y: [10, -5, 0], // Bounce effect
                transition: { delay: 0.3, duration: 1, ease: "easeOut", staggerChildren: 0.08 },
            });
            ...
            <div className="preloader-heading">
                <motion.div className="load-text" animate={textControls} initial={{ opacity: 0, y: 10 }}>
                    {["M", "u", "h", "a", "m", "m", "a", "d", " ", "A", "s", "i", "f"].map((char, index) => (
                        <motion.span key={index} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08, duration: 0.6 }}>
                            {char}
                        </motion.span>
                    ))}
                </motion.div>
